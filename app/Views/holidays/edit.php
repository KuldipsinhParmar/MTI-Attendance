...
<div class="card" style="max-width:720px;">
    <div class="card-header">
        <i class="bi bi-calendar-event me-2 text-primary"></i> Edit Holiday
    </div>
    <div class="card-body">
        <form action="<?= base_url('holidays/update/' . $holiday['id']) ?>" method="POST">
            <?= csrf_field() ?>
            <div class="row g-3">
                <div class="col-md-6">
                    <label class="form-label fw-medium">Holiday Name <span class="text-danger">*</span></label>
                    <input type="text" name="name" class="form-control" required
                           value="<?= old('name', esc($holiday['name'])) ?>" placeholder="e.g. New Year's Day">
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-medium">Date <span class="text-danger">*</span></label>
                    <input type="date" name="date" class="form-control" required
                           value="<?= old('date', esc($holiday['date'])) ?>">
                </div>
            </div>
